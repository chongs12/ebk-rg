...
func (h *Handler) Ask(c *gin.Context) {
    ...
    case string:
        id, err := uuid.Parse(v); if err != nil { c.JSON(http.StatusUnauthorized, gin.H{"error": "invalid user id"}); return }
        uid = id
    case uuid.UUID:
        uid = v
    default:
        c.JSON(http.StatusUnauthorized, gin.H{"error": "invalid user id type"}); return
    }
